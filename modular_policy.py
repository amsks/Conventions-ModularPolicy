...
class ModularPolicy(BasePolicy):
    ...
    def merge_partner_modules(self) -> None:
        self.partner_mlp_extractor = nn.ModuleList([self.partner_mlp_extractor[0]] * self.num_partners)
        self.partner_action_net = nn.ModuleList([self.partner_action_net[0]] * self.num_partners)
        self.partner_value_net = nn.ModuleList([self.partner_value_net[0]] * self.num_partners)

        # Setup optimizer with initial learning rate
        self.do_init_weights(init_main=False, init_partner=True)
